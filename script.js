...
// ---------- 8. Pantalla de carga + personalización con el nombre ----------

// Guardamos el nombre en localStorage: es memoria local del navegador,
// no una cuenta ni una base de datos real (eso llega en una fase más adelante).
const NAME_STORAGE_KEY = "nowly_name";

function showGreeting(name) {
  greetingEl.textContent = `GOOD TO SEE YOU, ${name.toUpperCase()}`;
}
...
function revealApp() {
  ...
  if (savedName) {
    ...
    // No tenemos nombre guardado todavía: mostramos el onboarding.
    onboarding.classList.remove("hidden");
    nameInput.focus();
  }
}

// Guarda el nombre y personaliza el saludo.
nameSubmitBtn.addEventListener("click", () => {
  const name = nameInput.value.trim();
  if (!name) return; // no dejamos avanzar con el campo vacío
...
// También permite confirmar con Enter, no solo con el botón.
...
// La pantalla de carga se queda un momento corto y luego se
// desvanece (el fundido en sí está definido en el CSS).
